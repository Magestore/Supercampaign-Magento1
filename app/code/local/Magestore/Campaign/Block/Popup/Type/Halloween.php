 ...
 */
class Magestore_Campaign_Block_Popup_Type_Halloween extends Magestore_Campaign_Block_Popup_Abstract
{
    ...
    public function _prepareLayout()
    {
        ...
        $this->setTemplate('campaign/popup/halloween.phtml');
        return $this;
    }
    ...
    public function getHtml($step = 1)
    {
        $popup = $this->getPopup();
        if(!$popup){
            $popup = $this->getCampaignPopup();
        }
        if(!$popup){
            return '';
        }
        $modelType = $popup->getModelType();
        if ($step == 1) {
            return $modelType->getContentStep1Html();
        } elseif ($step == 2) {
            return $modelType->getContentStep2Html();
        } elseif ($step == 3) {
            return $modelType->getContentStep3Html();
        }
        return '';
    }
